/**
 * Ported from src/components/explore/resultsParams.test.js. Targets P2's port
 * of explore/resultsParams.js, wherever under app/ or components/ it lands.
 */
...
interface ResultsParamsModule {
  readList: (params: URLSearchParams, key: string) => string[];
  resolveSort: (rawSort: string, hasQuery: boolean) => string;
  filtersFromParams: (params: FilterParams) => Filters;
  paramsFromFilters: (filters: Filters) => Record<'instrument' | 'format' | 'length', string | null>;
  pageWindow: (page: number, pages: number) => Array<number | null>;
}

const { filtersFromParams, pageWindow, paramsFromFilters, readList, resolveSort } = onlyModule(
  import.meta.glob<ResultsParamsModule>(
    ['/app/**/resultsParams.{ts,tsx,js,jsx}', '/components/**/resultsParams.{ts,tsx,js,jsx}'],
    { eager: true }
  ),
  'explore resultsParams'
);
...
describe('pageWindow', () => {
  ...
  it('always keeps the first and last page reachable', () => {
    for (const page of [1, 2, 50, 399, 400]) {
      const pages = pageWindow(page, 400);
