...
export type FeaturesVariant = 'notation' | 'stems' | 'midi';
 ...
 */
export default async function Features({ locale, variant = 'notation' }: { locale: Locale; variant?: FeaturesVariant }) {
  const t = await getT(locale);
  // The variant key resolves first and falls back to the shared one, so a
  // missing translation degrades to the notation copy rather than a key.
  const tv = (leaf: string, opts?: Record<string, string | number>) =>
