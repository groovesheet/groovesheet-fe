...
import type { BillingPlan, BillingTopup, UsageTransaction, UserSubscription, PaymentMethod } from '@/lib/types';
...
type PlanFamily = 'lite' | 'pro';

interface Plan extends BillingPlan {
  checkout_id?: string;
}
interface Topup extends BillingTopup {
  checkout_id?: string;
}
...
const tierFamily = (tier: string | null | undefined) => (tier || 'free').split('_')[0]; // free | lite | pro
const tierLabel = (tier: string) => TIER_LABELS[tier] || 'Free';
...
const CHECKOUT_CODE: Record<string, Record<BillingMode, string>> = {
  lite: { monthly: 'tier2', annual: 'tier2_annual' },
  pro: { monthly: 'tier3', annual: 'tier3_annual' },
};

const humanizeType = (type: string | undefined) => {
...
const fmtBalance = (m: number) => (Number.isInteger(m) ? String(m) : Number(m).toFixed(1));
...
const PLAN_FALLBACK: Record<PlanFamily, { name: string; badge: string; subtitle: string; monthly: number; annual: number; annualBilled: number; features: string[] }> = {
...
export default function AccountBilling() {
  ...
  const [catalog, setCatalog] = useState<CatalogState>({ plans: [], topups: [], provider: 'stripe', currency: 'usd' });
  ...
  // Each notify gets a fresh object, so repeating a message restarts its timer.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await fetchAccountSummary<AccountSummary>(config.apiBaseUrl, getToken, signOut);
        if (!cancelled) {
          setSummary(data);
        ...
        else setSummaryError(errorMessage(err) || "Couldn't load your plan");
      } finally {
        if (!cancelled) setSummaryLoading(false);
      }
    })();
    // Public catalog (no auth). The CRA page dropped `currency` here, so a
    // visitor quoted in CNY saw yuan amounts behind a dollar sign; keep it.
    fetchBillingPlans<CatalogResponse>(config.apiBaseUrl)
      .then((c) => {
        if (!cancelled) {
          setCatalog({ plans: c?.plans || [], topups: c?.topups || [], provider: c?.provider || 'stripe', currency: c?.currency || 'usd' });
        }
      })
      .catch(() => {});
    // Payment method is best-effort; the section hides if the endpoint is missing.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        else setUsageError(errorMessage(err) || "Couldn't load usage history");
        setTransactions([]);
      } finally {
  ...
  const money = (value: number | string | undefined) => `${currency === 'cny' ? '¥' : '$'}${value}`;
  ...
  const planMonthly = (p: Plan | undefined) => p?.price_monthly ?? p?.price_monthly_usd;
  const planAnnual = (p: Plan | undefined) => p?.price_annual ?? p?.price_annual_usd;
  ...
  const planMinutes = currentPlan?.minutes_per_month ?? 0;
  const allowance: number | null = planMinutes > 0 ? planMinutes : isFree ? null : Math.max(balanceMinutes, 1);
  ...
  const planCards = (['lite', 'pro'] as const).map((fam) => {
  ...
  const main: CSSProperties = { position: 'relative', zIndex: 1, maxWidth: 1454, margin: '0 auto', padding: '48px 20px 110px' };
  ...
  const label: CSSProperties = { fontFamily: 'var(--font-family-alt)', fontSize: 16, color: 'var(--color-muted-foreground)' };
  const h2: CSSProperties = { fontSize: 24, lineHeight: '32px', color: 'var(--color-foreground)', margin: 0, fontWeight: 400 };
  const muted: CSSProperties = { fontFamily: 'var(--font-family-alt)', fontSize: 15, color: 'var(--color-muted-foreground)' };
            ...
            <>
              ...
              <div id="plans-section" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <p style={{ ...muted, margin: 0 }}>Need a quick boost? Buy minutes that never expire, with no subscription change.</p>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
                  {topupCards.map((t) => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ ...label }}>
                    {total === 0 ? 'No transactions' : `${total} transactions · showing ${rangeStart} to ${rangeEnd}`}
                  </span>
                </div>
                          ...
                          {transactions.map((tx) => {
                            ...
                            return (
                              <tr key={tx.id}>
                                ...
                                <td style={td}>{tx.description || EMPTY}</td>
