...
import { ProcessingCard } from '@/components/ProcessingJobs';
import {
  apiPrefixForId,
...
interface LibraryWorkflow extends Workflow {
  workflow_id: string;
...
type StatusFilter = 'all' | WorkflowVisibility;
type Pop = { id: string; kind: 'publish' | 'delete' };

// Placeholder for an empty cell, as an escape so the source stays free of dash characters.
const EMPTY = '\u2014';
...
const isProcessing = (w: LibraryWorkflow) => PROCESSING_STATES.includes(w.status || '');
const isCompleted = (w: LibraryWorkflow) => COMPLETED_STATES.includes(w.status || '');
const isFailed = (w: LibraryWorkflow) => w.status === 'failed';
...
const visibilityOf = (w: LibraryWorkflow) => w.visibility || 'private';
...
// title already shows.
const kindOf = (w: LibraryWorkflow) => String(resolveDescription(w) || '').split(' · ')[0] || 'Transcription';
// The one line under the title: the user's own description if they wrote one,
// else "<kind> · 16:15 September 3, 2026".
const cardLine = (w: LibraryWorkflow, ds: string) => (w.description && String(w.description).trim())
...
const BassGlyph = ({ size }: { size: number }) => (
...
const InstrumentGlyph = ({ instrument, size }: { instrument: string; size: number }) => {
  switch (instrument) {
...
const downloadName = (w: LibraryWorkflow, kind: string, ext: string) => {
...
const EditDetailsModal = ({ workflow, onClose, onSave }: { workflow: LibraryWorkflow; onClose: () => void; onSave: (patch: DetailsPatch) => Promise<void> }) => {
  const initialArtist = workflow.original_artist || (typeof workflow.metadata?.artist === 'string' ? workflow.metadata.artist : '');
  ...
  const field: CSSProperties = { width: '100%', height: 44, background: 'var(--color-input-bg)', border: '1px solid var(--color-input-border)', borderRadius: 8, padding: '0 13px', fontFamily: 'var(--font-family-sans)', fontSize: 14, color: 'var(--color-text)', outline: 'none' };
...
// Popovers open upward on grid cards (the button sits at the card's foot) and
// downward in list rows.
const anchorAt = (anchorBottom: boolean): CSSProperties => (anchorBottom ? { bottom: 50 } : { top: 46 });
...
export default function TranscriptionHistory() {
  ...
  const [menuFor, setMenuFor] = useState<string | null>(null); // workflow_id with open kebab
  const [popFor, setPopFor] = useState<Pop | null>(null);
  const [editFor, setEditFor] = useState<LibraryWorkflow | null>(null); // workflow being edited
  const [removeFromExplore, setRemoveFromExplore] = useState(false); // delete-popover option
  const [pdfFor, setPdfFor] = useState<string | null>(null); // workflow_id whose PDF is rendering
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const listData = await fetchWorkflowList<LibraryListResponse | string[]>(config.apiBaseUrl, getToken, signOut, { limit: 100, offset: 0 });
        if (!Array.isArray(listData) && listData && Array.isArray(listData.items)) {
          if (!cancelled) setWorkflows(listData.items);
        } else {
          let ids: string[] = [];
          if (Array.isArray(listData)) ids = listData;
          else if (listData && Array.isArray(listData.workflow_ids)) ids = listData.workflow_ids;
          if (ids.length) {
            const statuses = await Promise.all(
              ids.map((id) =>
                fetchWorkflowStatus<LibraryWorkflow>(config.apiBaseUrl, id, getToken, signOut).catch((err: unknown) => {
                  if (isAuthError(err)) throw err;
                  return null;
                })
              )
            );
            if (!cancelled) setWorkflows(statuses.filter((s): s is LibraryWorkflow => Boolean(s)));
          } else if (!cancelled) {
            setWorkflows([]);
          }
        }
        ...
        if (isAuthError(err)) {
          ...
          setError(errorMessage(err) || "Couldn't load your library");
          setRetryAt(Date.now());
        }
      } finally {
  ...
  // The interval reads the latest list through a ref so a status change does
  // not tear the timer down and restart it.
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      const updates = await Promise.all(
        active.map((w) =>
          fetchWorkflowStatus<LibraryWorkflow>(config.apiBaseUrl, w.workflow_id, getToken, signOut).catch(() => null)
        )
      );
      if (cancelled) return;
      const byId: Record<string, LibraryWorkflow> = {};
      updates.forEach((u) => {
        if (u) byId[u.workflow_id] = u;
      });
  ...
  const handleDownload = async (workflowId: string, fileKey: string | undefined, fallbackName: string) => {
    setMenuFor(null);
    try {
      const token = await getToken();
      // Previews (PRV*) are served from /preview and may be owned by the
      // anonymous cookie session rather than the bearer, so send both.
  ...
  const handleDownloadPdf = async (workflowId: string, filename: string) => {
  ...
  const setVisibility = async (workflowId: string, visibility: WorkflowVisibility) => {
    ...
    try {
      const res = await updateWorkflowVisibility<VisibilityResult>(config.apiBaseUrl, workflowId, visibility, getToken, signOut);
      ...
      notify(errorMessage(err) || 'Could not change visibility');
    }
  };

  const saveDetails = async (workflowId: string, patch: DetailsPatch) => {
    const saved = await updateWorkflowMetadata<{ title?: string | null }>(config.apiBaseUrl, workflowId, patch, getToken, signOut);
    setWorkflows((ws) => ws.map((w) => (
  ...
  const doDelete = async (workflowId: string, deleteTrack: boolean) => {
    setPopFor(null);
    setMenuFor(null);
    try {
  ...
  const matchesQuery = (w: LibraryWorkflow) => resolveDisplayName(w).toLowerCase().includes(query.toLowerCase());
  const instrOf = (w: LibraryWorkflow) => w.metadata?.instrument || 'drums';
  ...
  // `sorted` is a fresh array every render, so a useMemo here never hit.
  const byYear: Record<string, LibraryWorkflow[]> = {};
  for (const w of sorted) {
    const ds = w.created_at || w.completed_at;
    const y = String(ds ? new Date(ds).getFullYear() : new Date().getFullYear());
  ...
  // ---- styles ----
  const page: CSSProperties = { position: 'relative', minHeight: '100vh', background: 'var(--color-background)', color: 'var(--color-text)', fontFamily: 'var(--font-family-sans)', WebkitFontSmoothing: 'antialiased' };
  const dots: CSSProperties = { position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', backgroundImage: 'radial-gradient(circle, var(--color-dot-pattern) 1.5px, transparent 1.5px)', backgroundSize: '43px 43px', opacity: 0.35 };
  // 1414 + 2x20 gutter: lines the content up with the nav bar (Header.css).
  const main: CSSProperties = { position: 'relative', zIndex: 1, maxWidth: 1454, margin: '0 auto', padding: '48px 20px 80px' };
  const toolbarField: CSSProperties = { height: 52, background: 'var(--color-input-bg)', border: '1px solid var(--color-input-border)', borderRadius: 8 };
  ...
  const groupTitle: CSSProperties = { fontSize: 22, lineHeight: '28px', fontWeight: 400, margin: 0, color: 'var(--color-foreground)' };
  const badge = (w: LibraryWorkflow) => {
    const v = visibilityOf(w);
    const map: Record<string, { label: string; color: string; dot?: boolean; lock?: boolean }> = {
  ...
  const Thumb = (w: LibraryWorkflow, h?: boolean) => {
    // Covers come from the API's own storage at arbitrary sizes; next/image
    // would need every origin whitelisted for no gain on a private page.
  ...
  const StatusBadge = (w: LibraryWorkflow) => {
    const b = badge(w);
    return (
  ...
  const InstrBadges = (w: LibraryWorkflow) => {
    const i = instrOf(w);
    return (
  ...
  const TypeBadge = (w: LibraryWorkflow) => {
  ...
  const PreviewTag = (w: LibraryWorkflow): ReactNode => (w.is_preview
    ? <span style={{ fontSize: 12, padding: '3px 8px', borderRadius: 999, border: '1px solid var(--color-border)', color: 'var(--color-muted-foreground)' }}>10s preview</span>
    : null);

  // Kebab menu (shared between grid + list).
  const KebabMenu = (w: LibraryWorkflow, anchorBottom: boolean) => {
    ...
    const menuStyle: CSSProperties = { position: 'absolute', right: 0, ...anchorAt(anchorBottom), zIndex: 60, width: 230, background: 'var(--color-panel1)', borderRadius: 10, padding: 6, boxShadow: '0 14px 40px rgba(0,0,0,.45)' };
    const item = (color: string): CSSProperties => ({ display: 'flex', alignItems: 'center', gap: 10, width: '100%', textAlign: 'left', background: 'none', border: 'none', fontFamily: 'var(--font-family-sans)', fontSize: 14, color, padding: '9px 10px', borderRadius: 6, cursor: 'pointer' });
    const dl: CSSProperties = { background: 'transparent', border: '1px solid var(--color-border)', color: 'var(--color-text)', fontFamily: 'var(--font-family-sans)', fontSize: 12, fontWeight: 500, padding: '7px 4px', borderRadius: 6, cursor: 'pointer' };
    return (
  ...
  const PublishPopover = (w: LibraryWorkflow, anchorBottom: boolean) => {
    const id = w.workflow_id;
    const cur = visibilityOf(w);
    const opts: Array<{ v: WorkflowVisibility; label: string; desc: string }> = [
    ...
    return (
      <div role="dialog" aria-modal="true" className="gs-pop" style={{ position: 'absolute', right: 0, ...anchorAt(anchorBottom), zIndex: 60, width: 300, background: 'var(--color-panel1)', borderRadius: 12, padding: 18, boxShadow: '0 14px 40px rgba(0,0,0,.45)', textAlign: 'left' }} onClick={(e) => e.stopPropagation()}>
  ...
  const DeletePopover = (w: LibraryWorkflow, anchorBottom: boolean) => {
    const published = ['public', 'unlisted'].includes(visibilityOf(w));
    return (
      <div role="dialog" aria-modal="true" className="gs-pop" style={{ position: 'absolute', right: 0, ...anchorAt(anchorBottom), zIndex: 60, width: 290, background: 'var(--color-panel1)', borderRadius: 12, padding: 18, boxShadow: '0 14px 40px rgba(0,0,0,.45)', textAlign: 'left' }} onClick={(e) => e.stopPropagation()}>
  ...
  const Card = (w: LibraryWorkflow) => {
    ...
    return (
      <div className="gs-card" key={w.workflow_id} style={{ background: 'var(--color-panel2)', borderRadius: 13, border: '1px solid transparent', overflow: 'visible', display: 'flex', flexDirection: 'column' }}>
        <Link href={href} aria-label={`Open ${name}`} style={{ position: 'relative', display: 'block', height: 172, borderRadius: '13px 13px 0 0', overflow: 'hidden', background: '#15151a' }}>
          {Thumb(w)}
          {dur && (
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 7, flex: 1 }}>
          <Link href={href} style={{ fontSize: 17, fontWeight: 500, color: 'var(--color-text)', lineHeight: '22px', textDecoration: 'none' }}>{base}<span style={{ color: 'var(--color-muted-foreground)' }}>{ext}</span></Link>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 2 }}>{TypeBadge(w)}{InstrBadges(w)}{StatusBadge(w)}{PreviewTag(w)}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 'auto', paddingTop: 10 }}>
            <Link href={href} className="gs-btn gs-btn-secondary" style={{ flex: 1 }}>
  ...
  const ListRow = (w: LibraryWorkflow) => {
    const name = resolveFileDisplayName(w);
    const { base, ext } = fileParts(name);
    const ds = w.created_at || w.completed_at || new Date().toISOString();
    const dur = fmtDuration(w.metadata?.duration_seconds ?? w.duration_seconds);
    ...
    return (
      <div className="gs-row" key={w.workflow_id} style={{ display: 'grid', gridTemplateColumns: 'minmax(150px,2.4fr) 1.4fr 1.5fr 0.9fr 0.6fr 88px', gap: 16, alignItems: 'center', padding: '12px 18px', borderBottom: '1px solid var(--color-border)' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', minWidth: 0 }}>{TypeBadge(w)}{InstrBadges(w)}{PreviewTag(w)}</div>
        ...
        <div style={{ fontSize: 13, color: 'var(--color-muted-foreground)', whiteSpace: 'nowrap' }}>{dur || EMPTY}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', position: 'relative' }}>
          <Link href={href} aria-label={`Open ${name}`} style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--color-border)', borderRadius: 8, color: 'var(--color-text)', textDecoration: 'none' }}>
    ...
    <div style={page} onClick={() => { setMenuFor(null); setPopFor(null); setSortOpen(false); }}>
      ...
      <main style={main}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap', marginBottom: 32 }}>
          ...
          <Link href="/" className="gs-btn gs-btn-primary" style={{ whiteSpace: 'nowrap' }}>
        ...
        {trulyEmpty && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 18, padding: '90px 20px', background: 'var(--color-panel2)', borderRadius: 13 }}>
            ...
            <Link href="/" className="gs-btn gs-btn-primary" style={{ marginTop: 6 }}>Transcribe your first song</Link>
          </div>
        )}
                  ...
                  {failed.map((w) => {
                    ...
                    return (
                      <div key={w.workflow_id} style={{ background: 'var(--color-panel2)', borderRadius: 13, padding: '22px 24px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                        ...
                        <button className="gs-btn gs-btn-secondary" onClick={(e) => { e.stopPropagation(); setRemoveFromExplore(false); setPopFor({ id: w.workflow_id, kind: 'delete' }); }} style={{ whiteSpace: 'nowrap' }}>Remove</button>
                        <div style={{ position: 'relative' }}>
