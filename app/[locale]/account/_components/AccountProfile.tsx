...
import { useAuth, useAuthActions, useUser } from '@/lib/auth';
...
/** GET /account/profile. `links` is a list; older payloads sent a platform map. */
interface OwnProfile {
  username?: string | null;
  display_name?: string | null;
  bio?: string | null;
  avatar_url?: string | null;
  links?: ProfileLink[] | Record<string, string | null>;
...
const TIER_LABELS: Record<string, string> = { free: 'Free', lite: 'Lite', pro: 'Pro' };
// Free tier has no monthly allowance: the 10-second preview is the free path.
const TIER_MINUTES: Record<string, number> = { free: 0, lite: 120, pro: 300 };
...
const tierFamily = (t: string | null | undefined) => (t || 'free').split('_')[0];
const tierLabel = (t: string) => TIER_LABELS[tierFamily(t)] || 'Free';
...
const fmtMonthYear = (d: string | null | undefined) => {
...
const linksOf = (profile: OwnProfile | null): ProfileLink[] => (Array.isArray(profile?.links) ? profile.links : []);
...
export default function AccountProfile() {
  ...
  const [planTotalMinutes, setPlanTotalMinutes] = useState<number | null>(null);
  const [planPrice, setPlanPrice] = useState('');

  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState<Draft>({ username: '', displayName: '', bio: '', links: [], firstName: '', lastName: '' });
  const [uState, setUState] = useState<{ checking: boolean; ok: boolean | null }>({ checking: false, ok: null });
  // JSON of the draft when editing began; the dirty check compares against it.
  const [original, setOriginal] = useState<string | null>(null);

  const [emailDraft, setEmailDraft] = useState('');
  const [namesOverride, setNamesOverride] = useState<Names | null>(null); // saved names until the session token refreshes
  ...
  const uTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoadError(null);
      try {
        const s = await fetchAccountSummary<AccountSummary>(config.apiBaseUrl, getToken, signOut);
        if (!cancelled) setSummary(s);
      } catch (err) {
        if (!cancelled && !handleAuthError(err)) setLoadError(errorMessage(err) || "Couldn't load your account");
      }
      let p: OwnProfile | null = null;
      try {
        p = await fetchCreatorProfile<OwnProfile>(config.apiBaseUrl, getToken, signOut);
      } catch {
        p = null;
      }
      if (!cancelled) {
        const fallbackName = user?.name || '';
        const createdAt = user?.user_metadata?.created_at;
        setProfile(
          p || {
            ...
            bio: '',
            links: [],
            member_since: typeof createdAt === 'string' ? createdAt : null,
          }
        );
        setLoading(false);
      }
  ...
  // Re-seed the email field from the account whenever the user or edit mode
  // changes, as the CRA page did.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // The availability badge is derived from the draft plus an async check;
    // resetting it here keeps both in one place, as the CRA page had it.
  ...
  const providersSet = useMemo(() => {
    const raw: Array<{ provider?: string }> = user?.external_accounts || user?.identities || [];
    return new Set(raw.map((p) => p.provider).filter((p): p is string => Boolean(p)));
  }, [user]);
  ...
  const subscription: UserSubscription = summary?.subscription || {};
  ...
  const readLinks = linksOf(profile)
  ...
  const startEdit = () => {
    const d: Draft = {
      ...
      bio: profile?.bio || '',
      links: linksOf(profile).map((l) => ({ label: l.label || l.platform || 'Website', url: l.url || '' })),
      firstName,
      lastName,
    };
    setOriginal(snapshot(d));
    setDraft(d);
    setUState({ checking: false, ok: null });
    setEditing(true);
  };
  ...
  const dirty = editing && original != null && snapshot(draft) !== original;
  ...
  const save = async () => {
    ...
    try {
      const updated = await updateCreatorProfile<OwnProfile>(config.apiBaseUrl, patch, getToken, signOut);
      setProfile((prev) => ({ ...prev, ...(updated || patch), links: updated?.links || patch.links }));
    } catch (err) {
      if (handleAuthError(err)) return;
      notify(errorMessage(err) || 'Saving your profile is not available yet');
      return;
    }
  ...
  const setLink = (i: number, field: keyof DraftLink, v: string) =>
  ...
  const downloadExport = async () => {
    ...
    try {
      ...
      saveBlob(blob, 'groovesheet-export.json');
    } catch (err) {
      if (!handleAuthError(err)) notify(errorMessage(err) || 'Could not export your data');
    } finally {
      setExporting(false);
    }
  };
  ...
  const onAvatarPick = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { avatar_url } = await uploadAvatar<OwnProfile>(config.apiBaseUrl, file, getToken, signOut);
      setProfile((prev) => ({ ...prev, avatar_url }));
      notify('Profile photo updated');
    } catch (err) {
      if (!handleAuthError(err)) notify(errorMessage(err) || 'Could not upload photo');
    } finally {
  ...
  const saveEmail = async () => {
    ...
    try {
      ...
      if (!handleAuthError(err)) notify(errorMessage(err) || 'Changing email is not available yet');
    }
  };

  // Same page in the chosen locale (query and hash kept); also records the
  // choice so `/` honours it next time.
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!handleAuthError(err)) notify(errorMessage(err) || 'Account deletion is not available yet');
    }
  };
  ...
  const page: CSSProperties = { position: 'relative', minHeight: '100vh', background: 'var(--color-background)', fontFamily: 'var(--font-family-sans)', WebkitFontSmoothing: 'antialiased', overflowX: 'hidden' };
  const dots: CSSProperties = { position: 'fixed', inset: 0, backgroundImage: 'radial-gradient(circle, var(--color-dot-pattern) 1.5px, transparent 1.5px)', backgroundSize: '43px 43px', opacity: 0.35, pointerEvents: 'none', zIndex: 0 };
  // 1414 + 2x20 gutter: lines the content up with the nav bar (Header.css).
  const main: CSSProperties = { position: 'relative', zIndex: 1, maxWidth: 1454, margin: '0 auto', padding: '48px 20px 96px' };
  const card: CSSProperties = { background: 'var(--color-panel2)', borderRadius: 13, padding: '26px 28px' };
  const h3: CSSProperties = { margin: 0, fontSize: 18, fontWeight: 500, color: 'var(--color-foreground)' };
  const label: CSSProperties = { fontSize: 13, fontWeight: 500, color: 'var(--color-text)' };
  const input: CSSProperties = { height: 42, padding: '0 13px', background: 'var(--color-input-bg)', border: '1px solid var(--color-input-border)', borderRadius: 6, color: 'var(--color-text)', fontFamily: 'var(--font-family-sans)', fontSize: 14, outline: 'none' };
  const rowRead: CSSProperties = { display: 'grid', gridTemplateColumns: '160px 1fr', gap: 20, padding: '14px 0', borderTop: '1px solid var(--color-border-light)' };
  ...
  // ---- signed-out ----
  // proxy.ts already sends signed-out visitors to the sign-in modal, so this
  // only shows when a session ends while the page is open.
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18, minWidth: 0 }}>
              ...
              <section id="sec-profile" style={{ scrollMarginTop: 84 }}>
                <div style={{ background: 'var(--color-panel1)', borderRadius: 11, padding: '26px 28px', display: 'flex', alignItems: 'center', gap: 22, flexWrap: 'wrap' }}>
                  ...
                  >
                    {profile?.avatar_url ? (
                      // User-uploaded avatar from the API's storage; next/image
                      // would need that origin whitelisted for no gain here.
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <Link href={`/u/${username}`} className="aps-link" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 11, fontSize: 13.5, fontWeight: 500, color: 'var(--color-foreground)', textDecoration: 'none', transition: 'opacity .2s ease' }}>
                      View public profile
                      ...
                      <span aria-live="polite" style={{ fontSize: 12, color: uState.ok === true ? 'var(--color-success)' : uState.ok === false ? 'var(--color-danger)' : muted }}>
                        {uState.checking
                          ...
                          : uState.ok === true
                            ...
                            : uState.ok === false
                              ? 'That username is unavailable. Use 3 to 20 letters, numbers, or underscores.'
                  ...
                  {providerRows.map((p, idx) => (
                    <div key={p.key} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', borderTop: idx ? '1px solid var(--color-border-light)' : 'none' }}>
                      <span style={{ width: 36, height: 36, borderRadius: 9, flex: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: p.key === 'facebook' ? 16 : 15, fontFamily: p.glyphFont, background: p.glyphBg, color: p.glyphColor }}>
                        {p.key === 'apple' ? <AppleGlyph size={17} /> : p.glyph}
                      </span>
                  ...
                  {!emailManaged ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 15px', background: 'var(--color-panel3)', borderRadius: 8 }}>
                      <span style={{ width: 30, height: 30, borderRadius: 8, flex: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, fontFamily: managedProvider?.glyphFont || 'inherit', background: managedProvider?.glyphBg || '#fff', color: managedProvider?.glyphColor || '#000' }}>
                        {managedProvider?.key === 'apple' ? <AppleGlyph size={15} /> : (managedProvider?.glyph || 'G')}
                      </span>
                      <div style={{ flex: 1, minWidth: 0 }}>
