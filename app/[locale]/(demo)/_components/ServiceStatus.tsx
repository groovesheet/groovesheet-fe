/**
 * /service-status: public system-status page. Ported from
...
import { ArrowClockwise, Bell, CheckCircle, Warning, Wrench, XCircle } from '@phosphor-icons/react';
...
// Shown where a figure is not available yet.
const NO_VALUE = '\u2014';

/** One worker as GET /service-status reports it. */
interface ServiceHealthRecord {
  ...
  min_consumer_idle_ms?: number | null;
  health?: {
    processing?: boolean;
    current_job?: string | null;
    processing_age_s?: number | null;
  } | null;
}
...
interface ServiceStatusResponse {
  ...
  recent?: { completed_24h?: number | null; failed_24h?: number | null };
}

type ServiceState = 'operational' | 'degraded' | 'down' | 'maintenance';
type OverallState = 'operational' | 'degraded' | 'down';
...
const STATUS: Record<ServiceState, { color: string; tint: string; icon: PhosphorIcon; label: string }> = {
...
const OVERALL: Record<OverallState, { key: OverallState; title: string }> = {
...
const fmtAge = (ms: number | null | undefined): string | null => {
...
function deriveService(
  s: ServiceHealthRecord,
  degradedNames: string[] | undefined
): { status: ServiceState; note: string } {
  if (!s.up) {
    return {
      status: 'down',
...
function Chip({ status }: { status: ServiceState }) {
...
function HealthStrip({ status }: { status: ServiceState }) {
...
function Skeleton() {
  const bar = (w: number | string, h: number, r: number | string = 8) => (
    <div
      style={{
        width: w,
        height: h,
...
export default function ServiceStatus() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetchServiceStatus<ServiceStatusResponse>();
  ...
  let overallKey: OverallState = 'operational';
  ...
  const successRate =
    completed != null && failed != null && completed + failed > 0
      ...
      : NO_VALUE;
  const tiles = [
    { label: 'Jobs in queue', value: data ? String(totalQueue) : NO_VALUE, sub: 'waiting to start' },
    { label: 'Completed', value: completed != null ? String(completed) : NO_VALUE, sub: 'last 24 hours' },
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 788, margin: '0 auto', padding: '0 20px' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 13, minWidth: 0 }}>
            {/* eslint-disable-next-line @next/next/no-img-element -- small theme-swapped logo, same as the CRA page */}
            <img
        ...
        {ready && (
          <>
            ...
            >
              ...
              >
                {(['operational', 'degraded', 'down', 'maintenance'] as const).map((k) => (
                  <span
                    key={k}
                    style={{
                      display: 'inline-flex',
