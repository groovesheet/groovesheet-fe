...
function maxPool(points: number[], bars: number): number[] {
 ...
 *
 * When `peaks` (the API's thumb_data.stems object: stem name to 200 ints) is
 ...
 */
function StemThumb({ song, peaks = null, width = 320, height = 192 }: ThumbProps) {
  const seed = hashStr(String(song.id) + '_stem');
  ...
  const hasPeaks = Boolean(peaks && Object.keys(peaks).length > 0);
  const stems =
    peaks && hasPeaks
  ...
  const proceduralBars = (kind: string) => {
    const bars: number[] = [];
  ...
  const renderWave = (yMid: number, color: string, kind: string) => {
    const stemPeaks = peaks ? peaks[kind.toLowerCase()] : undefined;
    const bars =
      hasPeaks && Array.isArray(stemPeaks) && stemPeaks.length > 0 ? maxPool(stemPeaks, SAMPLES) : proceduralBars(kind);
