...
import type { ThumbProps } from './resolveThumb';
...
type SheetGlyph = NoteHead | Beam;

function SheetThumb({ song, width = 320, height = 192 }: ThumbProps) {
  const seed = hashStr(String(song.id));
  // Per render, not memoized: see MidiThumb.
  ...
  const renderStaff = (yTop: number) => {
    const lines: ReactElement[] = [];
    for (let i = 0; i < 5; i++) {
      const y = yTop + lineGap * (i + 0.5);
      lines.push(<line key={'l' + i} x1={padX} x2={W - padX} y1={y} y2={y} stroke="#111" strokeWidth="0.6" />);
    }
    return lines;
  };
  ...
  const dens = (song.difficulty && DENSITY[song.difficulty]) || 0.85;
  const trebleY = padY;
  const bassY = padY + systemHeight + sysGap;

  const stepToY = (yTop: number, step: number) => yTop + lineGap * 2.5 - step * (lineGap / 2);

  const buildNotes = (yTop: number, range: number, beamy: boolean): SheetGlyph[] => {
    const els: SheetGlyph[] = [];
    for (let b = 0; b < barCount; b++) {
      ...
      for (let n = 0; n < beats; n++) {
        ...
        els.push({ x, y, step, eighth: isEighth });
        if (isEighth) {
          pair.push({ x, y });
          if (pair.length === 2) {
  ...
  const renderNotes = (notes: SheetGlyph[]) =>
    notes.map((n, i) => {
      if (n.beam) {
  ...
  const sig = (yTop: number) => (
    <g transform={`translate(${padX + 24}, ${yTop + lineGap * 2.5})`} fill="#111" fontFamily="serif" fontWeight="700">
      <text x="0" y="-1" fontSize={lineGap * 1.5} textAnchor="middle">
        4
      </text>
      <text x="0" y={lineGap * 1.4} fontSize={lineGap * 1.5} textAnchor="middle">
        4
      </text>
    </g>
  );
