...
import { ROLL_COLORS } from '@/lib/exploreConstants';
...
function MidiThumb({ song, width = 320, height = 192 }: ThumbProps) {
  const seed = hashStr(String(song.id) + '_midi');
  // Created per render (not memoized) so the sequence always starts from the
  // seed: the server and the hydrating browser must draw the same notes.
