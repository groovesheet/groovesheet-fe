// Shared constants + tiny formatters for the Explore page, the song page and
// creator profiles. Ported from src/components/explore/constants.js into lib
// because lib/creatorApi.ts needs them and lib may not import from components.
...
export const FORMAT_FILTER_MAP: Record<string, string> = {
...
export const lengthBucket = (sec: number | null | undefined): string => {
  const s = sec || 0;
...
export const capitalize = <T extends string | null | undefined>(s: T): T =>
...
export const FORMAT_PARAM_BY_LABEL: Record<string, string> = {
  'Sheet Music': 'sheet',
  MIDI: 'midi',
  Stems: 'stems',
};

export const FORMAT_LABEL_BY_PARAM: Record<string, string> = Object.fromEntries(
...
export const FORMAT_LABEL_BY_ASSET_TYPE: Record<string, string> = Object.fromEntries(
  Object.entries(FORMAT_FILTER_MAP).map(([label, assetType]) => [assetType, label])
);

/** Sidebar "Length" label → `length=` param value. */
export const LENGTH_PARAM_BY_LABEL: Record<string, string> = {
  'Under 2 min': 'under2',
  '2 to 5 min': '2to5',
  '5+ min': 'over5',
};

export const LENGTH_LABEL_BY_PARAM: Record<string, string> = Object.fromEntries(
...
export const VARIANT_VIEW: Record<string, string> = {
  sheet: 'sheet',
  midi: 'midi',
  stems: 'stems',
};
...
export function songPath(id: string, variant?: string | null): string {
  const view = variant ? VARIANT_VIEW[variant] : undefined;
