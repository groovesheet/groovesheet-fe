// DOM-based (DOMParser/XMLSerializer): browser only, and a no-op on the server.

const PERCUSSION_BY_NOTATION: Record<string, [number, string]> = {
...
function addPercussionPlaybackMetadata(doc: XMLDocument): void {
  ...
  root.querySelectorAll('part-list > score-part').forEach((scorePart) => {
    ...
    const used = new Map<string, { midiKey: number; name: string }>();
...
export function applyMusicXmlMetadata(xmlString: string, { title, artist, sourceCredit }: MusicXmlMetadata = {}): string {
  ...
  try {
    ...
    const ensureChild = (parent: Element, name: string, beforeSelector?: string): Element => {
    ...
    const upsertCreator = (type: string, value: string | null | undefined) => {
